...
```text
receiver output device: BlackHole 2ch
...
total_buf / remote_total ~= fixed + outqの48kHz換算frames
```

`--fixed-delay-frames 14400` かつ `--output-ring-ms 60` の場合、receiver 内部の実効遅延はおおむね `14400fr + output ringの48kHz換算frames + backend/device遅延` です。DAW 側で固定補正する場合は、クリックやパルスで実測して補正値を決めてください。
