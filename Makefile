...
SENDER_FEEDBACK_HOST ?= 192.168.11.96
RECEIVER_FEEDBACK_TARGET ?= $(SENDER_FEEDBACK_HOST):$(FEEDBACK_PORT)
# 480-10ms 4800-100ms
# 960-20 1440-30ms 2880-60ms
#DIRECT_FIXED_DELAY_FRAMES ?= 1440
#DIRECT_FIXED_DELAY_FRAMES ?= 360
DIRECT_FIXED_DELAY_FRAMES ?= 3600
...
DIRECT_SENDER_ARGS = --target $(SENDER_TARGET) $(SENDER_FEEDBACK_ARGS) $(SENDER_DEVICE_ARG) --packet-ms $(DIRECT_PACKET_MS) --capture-queue-capacity $(DIRECT_CAPTURE_QUEUE_CAPACITY)
...
help:
	...
	@printf '%s\n' '  RECEIVER_FEEDBACK_TARGET=<Windows sender LAN IP>:$(FEEDBACK_PORT)'
	@printf '%s\n' '  SENDER_INPUT_DEVICE="BlackHole 2ch"'
